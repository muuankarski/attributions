...
opts_chunk$set(echo=FALSE,eval=TRUE, cache=TRUE,message=FALSE,warning=FALSE,fig.width=15,fig.height=11)
```
...
ggplot(plotMap, aes(long,lat,group=group)) +
  ...
  geom_polygon(data = plotMap, aes(long,lat), 
               fill=NA, color = "white", size=0.1) +
  coord_map(project="orthographic") +
  scale_fill_gradient2(low=muted("red"), high=muted("blue"), midpoint=0.7570) +
  theme(legend.position="top") +
  theme(legend.text = element_text(size=14))
```
