...
library(knitr)
knit2html('regressionModelling.Rmd', options = c('toc', markdown::markdownHTMLOptions(TRUE)))
```
...
SBperGini <- glmer(pov.log.social ~ incsour3+edu2+income2+
                        past.diff+future.diff+crise+
                        perGini+gdpchange07.10+wbgi_vae+(1|cntry),
                       family = binomial(logit),
                       data = df.model)
IBperGini <- glmer(pov.log.individual ~ incsour3+edu2+income2+
                        past.diff+future.diff+crise+
                        perGini+gdpchange07.10+wbgi_vae+(1|cntry),
                       family = binomial(logit),
                       data = df.model)
