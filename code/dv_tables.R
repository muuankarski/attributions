...
t <- subset(t, Freq > 0)
t2 <-  dcast(t, group_general+cntry ~ q309_rec, value.var="Freq")
names(t2) <-  c("group_general","country","social.blame","individual.blame","individual.fate","social.fate")
t2 <- t2[order(t2$group,-t2$social.blame),]
########
y <- data.frame(prop.table(svytable(~group_general+q309_rec, d.df), 1)*100)
y$Freq <- round(y$Freq, 2)
y$country <- c("CEE Total","CIS Total","Western Europe Total")
y2 <-  dcast(y, group_general+country ~ q309_rec, value.var="Freq")
names(y2) <-  c("group_general","country","social.blame","individual.blame","individual.fate","social.fate")
#
#
library(plyr)
